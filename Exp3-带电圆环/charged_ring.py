...
def calculate_potential_on_grid(y_coords, z_coords):
    ...
    #    注意维度顺序和 phi 的积分点数
    phi_grid, y_grid, z_grid = np.meshgrid(np.linspace(0, 2 * np.pi, 100), y_coords, z_coords, indexing='ij')

    # 2. 计算场点到圆环上各点的距离 R
    #    圆环方程: x_s = a*cos(phi), y_s = a*sin(phi), z_s = 0
    #    场点: (0, y_grid, z_grid)
    R = np.sqrt((a * np.cos(phi_grid)) ** 2 + (a * np.sin(phi_grid) - y_grid) ** 2 + z_grid ** 2)
    ...
    V = np.trapz(dV, np.linspace(0, 2 * np.pi, 100), axis=0)
...
def plot_potential_and_field(y_coords, z_coords, V, Ey, Ez, y_grid, z_grid):
    ...
    stream_plot = plt.streamplot(y_grid, z_grid, Ey, Ez, color=E_magnitude, cmap='plasma', linewidth=1, density=1.5, arrowstyle='->')
    #    - 设置坐标轴标签和标题
    plt.xlabel('y / a')
    plt.ylabel('z / a')
